...
本文定義了與 AI 助手協作的最高指導原則，確保程式碼質量、可維護性和項目目標的一致性。
...
- **原則**：優先使用真實資料和實際分析，而非模式符合或假設
- **實踐**：
  - 直接分析圖片內容，不依賴檔案名或元資料
  - 優先使用視覺模型提取的結果（image_title），而非生成的推薦名稱
  - 建立完整的追蹤和驗證機制
- **反例**：使用檔案名模式符合而非真實分析，導致命名不準確
...
- ✅ 優先考慮本地化方案（隱私、成本、性能）
- ✅ 建立驗證機制來檢驗假設
- **範例**：選擇 Qwen3-VL 30B 本地模型而非 Claude API，原因是隱私保護、成本最佳化、無速率限制
...
場景 4：程式化整合（開發者）
  ...
  工具：full_batch_rename_execute.py（可作為模組）
```
...
### 本項目應用了以下原則
1. **真實性優先**：使用 Vision Model 直接分析圖片，而非模式符合
2. **可復用性**：提供通用的分析腳本和組態模板
...
**所有技術決策 MUST 透過 Context7 MCP 取得最新官方檔案。**

### 使用流程

```
1. 先呼叫 resolve-library-id 取得正確的 library ID
2. 再呼叫 context7-query-docs 取得檔案
3. 基於官方檔案做出技術決策
4. 在 commit 中記錄檔案來源
```
...
3. 基於官方檔案最佳實踐編寫腳本
4. 在 commit 中記錄來源：
   "基於 Bash 官方檔案 context7:/bminor/bash 的最佳實踐"
```
