# 功能規格：圖片智慧命名系統（Image Title-First Naming System）
...
> **專案定位**：圖片智慧命名系統是一個「自動化圖片分析和命名工具」，使用本地 Vision Model 智慧提取圖片標題，實現精準、自描述的檔案命名。無需手動輸入、無 API 成本、無隱私風險。
...
### User Story 1 - 一鍵智慧命名（Priority: P1）
...
2. **Given** 開發者需自動化整合，**When** 呼叫 Python API，**Then** 返回結構化結果（JSON 格式）
...
- **FR-016**: 系統 MUST 支援本地化組態（config.yaml 定義優先級、語言、分隔符等）
...
- **FR-022**: 系統 MUST 在檔案系統和日誌中統一使用繁體中文
...
- **FR-027**: 系統 MUST 支援批量處理（每批 10 張，可組態）
...
- **Config（組態）**：代表系統組態（config.yaml），包含 LM Studio 連線、批次大小、命名規則、驗證設定
...
│    └─ 副函式：is_already_renamed()                    │
│ 3️⃣分析: 批量調用 Qwen3-VL 視覺模型                    │
│    └─ 副函式：encode_image_to_base64(), query_qwen() │
│ 4️⃣規劃: 生成重命名計畫、檢測重複                      │
│    └─ 副函式：detect_duplicates(), add_suffix()       │
│ 5️⃣執行: 逐檔案重命名、刪除原檔案                      │
│    └─ 副函式：rename_file(), delete_original()        │
│ 6️⃣報告: 生成詳細執行報告和映射表                      │
│    └─ 副函式：generate_report()                       │
└─────────────────────────────────────────────────────────┘
                             ↓
...
- **SC-011**: 品質評分達到 100 / 100（程式碼、文檔、測試覆蓋）
