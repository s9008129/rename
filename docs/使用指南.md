...
**版本**：v1.2  
**最後更新**：2026-01-24（v1.2：進度追蹤和大量文件支持）

### ✨ v1.2 新增特性（2026-01-24）

本版本針對大量圖片的場景進行了重大改進：

| 特性 | 說明 | 效果 |
|-----|------|------|
| 🚀 **智能進度追蹤** | 實時顯示分析和重命名進度 | 用戶能看到系統運行進度 |
| ⏱️ **超時時間增加** | 從 1 小時增加到 24 小時 | 支持 2000+ 張圖片 |
| 📝 **詳細進度日誌** | 每個階段都有完整的日誌記錄 | 便於調試和監督 |
| 📊 **ETA 時間估計** | 動態估計剩餘完成時間 | 用戶知道需要等待多久 |
| 💾 **進度持久化** | 進度保存到文件 | 支持中斷後恢復（未來版本） |

**支持規模**：✅ 2449 張照片（iPhone 備份）| ✅ 前 10 張圖片驗證成功 | ✅ 實時進度反饋 
